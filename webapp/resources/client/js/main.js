...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const toast = document.getElementById('handleAddToCart'); // Sử dụng id mới

    // Kiểm tra xem toast có tồn tại không
    if (!toast) {
        console.error('Không tìm thấy phần tử #handleAddToCart trong DOM');
        return;
    }
    ...
    function showToast(heading, text, type = 'success') {
        console.log('showToast called:', { heading, text, type }); // Debug
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                toast.style.opacity = '1'; // Reset opacity
            }, 500);
        }, 5000);
    }
...
//                 cart.splice(index, 1);
//                 localStorage.setItem('cart', JSON.stringify(cart));
//                 renderCart(); // Render lại toàn bộ giỏ hàng
//             });
//         });
//     }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('productBrand').textContent = "Dell"; //hard code
...
//darkmode
// Lấy nút chuyển đổi và biểu tượng
const themeToggle = document.getElementById('theme-toggle');
const themeIcon = document.getElementById('theme-icon');

// Hàm để đặt giao diện
function setTheme(theme) {
    document.documentElement.setAttribute('data-bs-theme', theme);
    localStorage.setItem('theme', theme);
    // Cập nhật biểu tượng
    themeIcon.classList.remove(theme === 'dark' ? 'fa-sun' : 'fa-moon');
    themeIcon.classList.add(theme === 'dark' ? 'fa-moon' : 'fa-sun');
}

// Kiểm tra giao diện đã lưu trong localStorage
const savedTheme = localStorage.getItem('theme');
if (savedTheme) {
    setTheme(savedTheme);
} else {
    // Nếu không có giao diện lưu trữ, kiểm tra tùy chọn hệ thống
